...
Examples:
  ...
  node_service_postgres export --format yaml --output ./backup

  # Export specific resource types
  node_service_postgres export --kinds Node --output ./node-backup
`,
...
func exportResourceKind(ctx context.Context, kind, output, format string, perType bool) (int, error) {
	...
	switch kind {
	case "Node":
		items, e := storage.Querynodes(ctx).All(ctx)
		if e != nil {
			return 0, fmt.Errorf("failed to query nodes: %w", e)
		}
