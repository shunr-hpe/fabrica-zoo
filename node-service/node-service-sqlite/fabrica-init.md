# node-service-sqlite generation
...
.\fabrica-init\bin\fabrica-init.exe `
  --module github.com/openchami/node-service-sqlite `
  --group node-service.openchami.org `
  --db sqlite `
  --auth `
  --name node-service-sqlite `
...
```text
+ ~/work/p/fabrica/bin/fabrica.exe init node-service-sqlite --module github.com/openchami/node-service-sqlite --group node-service.openchami.org --storage-version v1 --storage-type ent --db sqlite --auth  (in fabrica-zoo/node-service)
🚀 Creating node-service-sqlite project...
...
go: downloading github.com/getkin/kin-openapi v0.148.0
...
```powershell
# in fabrica-zoo/node-service/node-service-sqlite/
$env:GOTOOLCHAIN = "go1.26.6"
go build ./...   # exit 0
```
