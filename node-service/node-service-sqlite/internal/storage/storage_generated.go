...
// LoadAllNodes loads all Node resources from Ent storage
func LoadAllNodes(ctx context.Context) ([]*v1.Node, error) {
	...
	entResources, err := entClient.Resource.Query().
		Where(entresource.KindEQ("Node")).
		WithLabels().
		WithAnnotations().
		All(ctx)
	if err != nil {
		return nil, fmt.Errorf("failed to load Node resources: %w", err)
	}

	// Convert to Fabrica resources
	var resources []*v1.Node
	for _, entResource := range entResources {
		...
		resources = append(resources, fabricaResource.(*v1.Node))
	}

	return resources, nil
}

// LoadNode loads a single Node resource by UID from Ent storage
func LoadNode(ctx context.Context, uid string) (*v1.Node, error) {
	...
	return fabricaResource.(*v1.Node), nil
}

// SaveNode saves a Node resource to Ent storage
func SaveNode(ctx context.Context, resource *v1.Node) error {
	if entClient == nil {
	...
	if err != nil {
		return fmt.Errorf("failed to convert Node to ent: %w", err)
	}
...
// DeleteNode deletes a Node resource from Ent storage
...
func (b *entStorageBackend) LoadAll(ctx context.Context, resourceType string) ([]json.RawMessage, error) {
	...
	switch resourceType {
	case "Node":
		resources, err := LoadAllNodes(ctx)
		if err != nil {
			return nil, err
		}
