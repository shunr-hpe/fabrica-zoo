...
//
// This file provides a Cobra-based CLI for the user_service_postgres API.
...
//   --server       Server URL (env: USER_SERVICE_POSTGRES_SERVER)
//   --timeout      Request timeout (env: USER_SERVICE_POSTGRES_TIMEOUT)
//   --output, -o   Output format: table, json, yaml (env: USER_SERVICE_POSTGRES_OUTPUT)
//   --version, -v  API version to request: v1, v2beta1, etc. (env: USER_SERVICE_POSTGRES_VERSION)
//   --token        JWT bearer token (env: USER_SERVICE_POSTGRES_TOKEN)
//   --config       Config file path (default: ~/.user_service_postgres-cli.yaml)
//
// Configuration sources (in order of precedence):
//   1. Command-line flags
//   2. Environment variables (USER_SERVICE_POSTGRES_*)
//   3. Config file (~/.user_service_postgres-cli.yaml)
//   4. Default values
//
// Usage examples:
...
//   export USER_SERVICE_POSTGRES_SERVER=https://user_service_postgres.example.com
//   export USER_SERVICE_POSTGRES_VERSION=v2beta1
//   client user list
//
...
import (
	...
	"github.com/openchami/user-service-postgres/pkg/client"
...
var rootCmd = &cobra.Command{
	...
	Short: "user_service_postgres CLI",
	Long:  `A command-line interface for managing user_service_postgres resources.`,
}
...
func init() {
	...
	// Global flags
	rootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is $HOME/.user_service_postgres-cli.yaml)")
	rootCmd.PersistentFlags().StringVar(&serverURL, "server", "http://localhost:8080", "user_service_postgres server URL")
	...
	viper.SetEnvPrefix("USER_SERVICE_POSTGRES")
	viper.AutomaticEnv()
...
func initConfig() {
	if cfgFile != "" {
		...
		viper.SetConfigName(".user_service_postgres-cli")
	}
