...
Examples:
  ...
  named_resource_service export --format yaml --output ./backup

  # Export specific resource types
  named_resource_service export --kinds Named --output ./named-backup
`,
...
func exportResourceKind(ctx context.Context, kind, output, format string, perType bool) (int, error) {
	...
	switch kind {
	case "Named":
		items, e := storage.Querynameds(ctx).All(ctx)
		if e != nil {
			return 0, fmt.Errorf("failed to query nameds: %w", e)
		}
