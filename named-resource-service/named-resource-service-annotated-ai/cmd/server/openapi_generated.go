...
<head>
    <meta charset="UTF-8">
    <title>named_resource_service API Documentation</title>
...
func GenerateOpenAPISpec() *openapi3.T {
	spec := &openapi3.T{
		OpenAPI: "3.0.0",
		Info: &openapi3.Info{
			Title:       "named_resource_service API",
	...
	healthExample := map[string]interface{}{
		"status":  "ok",
		"service": "named_resource_service",
		"version": "1.0.0",
...
// registerNamedPaths registers OpenAPI paths for Named resources
func registerNamedPaths(spec *openapi3.T) {
	// Generate schemas from Go types - NO ANNOTATIONS NEEDED
	resourceSchema, _ := openapi3gen.NewSchemaRefForValue(&v1.Named{}, spec.Components.Schemas)
	spec.Components.Schemas["Named"] = resourceSchema

	createReqSchema, _ := openapi3gen.NewSchemaRefForValue(&CreateNamedRequest{}, spec.Components.Schemas)
	spec.Components.Schemas["CreateNamedRequest"] = createReqSchema

	updateReqSchema, _ := openapi3gen.NewSchemaRefForValue(&UpdateNamedRequest{}, spec.Components.Schemas)
	spec.Components.Schemas["UpdateNamedRequest"] = updateReqSchema

	statusSchema, _ := openapi3gen.NewSchemaRefForValue(&v1.NamedStatus{}, spec.Components.Schemas)
	spec.Components.Schemas["NamedStatus"] = statusSchema
	...
	// List Nameds operation
	listOp := openapi3.NewOperation()
	listOp.OperationID = "listNameds"
	listOp.Summary = "List all Named resources"
	listOp.Description = "Returns a list of all Named resources in the inventory"
	listOp.Tags = []string{"Named"}
	listOp.Responses = openapi3.NewResponses()
	arraySchema := openapi3.NewArraySchema()
	arraySchema.Items = &openapi3.SchemaRef{Ref: "#/components/schemas/Named"}
	...
	// Create Named operation
	createOp := openapi3.NewOperation()
	createOp.OperationID = "createNamed"
	createOp.Summary = "Create a new Named resource"
	createOp.Description = "Creates a new Named resource with the provided specification"
	createOp.Tags = []string{"Named"}
	createOp.RequestBody = &openapi3.RequestBodyRef{
		Value: openapi3.NewRequestBody().
			WithRequired(true).
			WithJSONSchemaRef(&openapi3.SchemaRef{
				Ref: "#/components/schemas/CreateNamedRequest",
			}),
	}
	...
	getOp.OperationID = "getNamed"
	getOp.Summary = "Get a specific Named resource"
	getOp.Description = "Returns details of a specific Named resource by UID"
	getOp.Tags = []string{"Named"}
	...
	// Update Named operation
	updateOp := openapi3.NewOperation()
	updateOp.OperationID = "updateNamed"
	updateOp.Summary = "Update a Named resource"
	updateOp.Description = "Updates an existing Named resource with new values"
	updateOp.Tags = []string{"Named"}
	updateOp.RequestBody = &openapi3.RequestBodyRef{
		Value: openapi3.NewRequestBody().
			WithRequired(true).
			WithJSONSchemaRef(&openapi3.SchemaRef{
				Ref: "#/components/schemas/UpdateNamedRequest",
			}),
	}
	...
	// Patch Named operation
	patchOp := openapi3.NewOperation()
	patchOp.OperationID = "patchNamed"
	patchOp.Summary = "Patch a Named resource"
	patchOp.Description = "Partially updates an existing Named resource using patch semantics"
	patchOp.Tags = []string{"Named"}
	...
	// Update Named status operation
	updateStatusOp := openapi3.NewOperation()
	updateStatusOp.OperationID = "updateNamedStatus"
	updateStatusOp.Summary = "Update Named status"
	updateStatusOp.Description = "Updates only the status subresource for an existing Named"
	updateStatusOp.Tags = []string{"Named"}
	updateStatusOp.RequestBody = &openapi3.RequestBodyRef{
		Value: openapi3.NewRequestBody().
			WithRequired(true).
			WithJSONSchemaRef(&openapi3.SchemaRef{Ref: "#/components/schemas/NamedStatus"}),
	}
	...
	updateStatusOp.Responses.Set("200", &openapi3.ResponseRef{
		Value: openapi3.NewResponse().
			...
			WithJSONSchemaRef(&openapi3.SchemaRef{Ref: "#/components/schemas/Named"}),
	})
	...
	// Patch Named status operation
	patchStatusOp := openapi3.NewOperation()
	patchStatusOp.OperationID = "patchNamedStatus"
	patchStatusOp.Summary = "Patch Named status"
	patchStatusOp.Description = "Partially updates only the status subresource for an existing Named"
	patchStatusOp.Tags = []string{"Named"}
	...
	deleteOp.OperationID = "deleteNamed"
	deleteOp.Summary = "Delete a Named resource"
	deleteOp.Description = "Removes a Named resource from the inventory"
	deleteOp.Tags = []string{"Named"}
	...
	uidParam := openapi3.NewPathParameter("uid").
		WithDescription("Unique identifier of the Named resource").
		WithRequired(true).
	...
	// Add paths to spec
	spec.Paths.Set("/nameds", collectionPath)
	spec.Paths.Set("/nameds/{uid}", itemPath)
	spec.Paths.Set("/nameds/{uid}/status", statusPath)
}
