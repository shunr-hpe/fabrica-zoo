...
// The NamedFunc type is an adapter to allow the use of ordinary
// function as Named mutator.
type NamedFunc func(context.Context, *ent.NamedMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f NamedFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.NamedMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.NamedMutation", m)
}
