...
import (
	...
	"github.com/openchami/named-resource-service/internal/storage/ent/migrate"
...
type Client struct {
	...
	Label *LabelClient
	// Named is the client for interacting with the Named builders.
	Named *NamedClient
...
func (c *Client) init() {
	...
	c.Named = NewNamedClient(c.config)
...
func (c *Client) Intercept(interceptors ...Interceptor) {
	...
	c.Named.Intercept(interceptors...)
...
// NamedClient is a client for the Named schema.
type NamedClient struct {
	config
}

// NewNamedClient returns a client for the Named from the given config.
func NewNamedClient(c config) *NamedClient {
	return &NamedClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `named.Hooks(f(g(h())))`.
func (c *NamedClient) Use(hooks ...Hook) {
	c.hooks.Named = append(c.hooks.Named, hooks...)
}

// Intercept adds a list of query interceptors to the interceptors stack.
// A call to `Intercept(f, g, h)` equals to `named.Intercept(f(g(h())))`.
func (c *NamedClient) Intercept(interceptors ...Interceptor) {
	c.inters.Named = append(c.inters.Named, interceptors...)
}

// Create returns a builder for creating a Named entity.
func (c *NamedClient) Create() *NamedCreate {
	mutation := newNamedMutation(c.config, OpCreate)
	return &NamedCreate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// CreateBulk returns a builder for creating a bulk of Named entities.
func (c *NamedClient) CreateBulk(builders ...*NamedCreate) *NamedCreateBulk {
	return &NamedCreateBulk{config: c.config, builders: builders}
}

// MapCreateBulk creates a bulk creation builder from the given slice. For each item in the slice, the function creates
// a builder and applies setFunc on it.
func (c *NamedClient) MapCreateBulk(slice any, setFunc func(*NamedCreate, int)) *NamedCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &NamedCreateBulk{err: fmt.Errorf("calling to NamedClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*NamedCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
		setFunc(builders[i], i)
	}
...
// Update returns an update builder for Named.
func (c *NamedClient) Update() *NamedUpdate {
	mutation := newNamedMutation(c.config, OpUpdate)
	return &NamedUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOne returns an update builder for the given entity.
func (c *NamedClient) UpdateOne(_m *Named) *NamedUpdateOne {
	mutation := newNamedMutation(c.config, OpUpdateOne, withNamed(_m))
	return &NamedUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *NamedClient) UpdateOneID(id int) *NamedUpdateOne {
	mutation := newNamedMutation(c.config, OpUpdateOne, withNamedID(id))
	return &NamedUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// Delete returns a delete builder for Named.
func (c *NamedClient) Delete() *NamedDelete {
	mutation := newNamedMutation(c.config, OpDelete)
	return &NamedDelete{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// DeleteOne returns a builder for deleting the given entity.
func (c *NamedClient) DeleteOne(_m *Named) *NamedDeleteOne {
	return c.DeleteOneID(_m.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *NamedClient) DeleteOneID(id int) *NamedDeleteOne {
	builder := c.Delete().Where(named.ID(id))
	builder.mutation.id = &id
	builder.mutation.op = OpDeleteOne
	return &NamedDeleteOne{builder}
}

// Query returns a query builder for Named.
func (c *NamedClient) Query() *NamedQuery {
	return &NamedQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeNamed},
		inters: c.Interceptors(),
	}
}

// Get returns a Named entity by its id.
func (c *NamedClient) Get(ctx context.Context, id int) (*Named, error) {
	return c.Query().Where(named.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *NamedClient) GetX(ctx context.Context, id int) *Named {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// Hooks returns the client hooks.
func (c *NamedClient) Hooks() []Hook {
	return c.hooks.Named
}

// Interceptors returns the client interceptors.
func (c *NamedClient) Interceptors() []Interceptor {
	return c.inters.Named
}

func (c *NamedClient) mutate(ctx context.Context, m *NamedMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&NamedCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&NamedUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&NamedUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&NamedDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown Named mutation op: %q", m.Op())
	}
}
...
type (
	hooks struct {
		Annotation, Label, Named, Resource []ent.Hook
	}
	inters struct {
		Annotation, Label, Named, Resource []ent.Interceptor
	}
)
