# user-service-pr-postgres generation
...
.\fabrica-init\bin\fabrica-init.exe `
  --module github.com/openchami/user-service-pr-postgres `
  --group storage-annotations.openchami.org `
  --db postgres `
  --auth `
  --name user-service-pr-postgres `
...
- `--db postgres` selects the PostgreSQL Ent driver (pulls in `github.com/lib/pq`).
...
```text
+ ~/work/p/fabrica/bin/fabrica.exe init user-service-pr-postgres --module github.com/openchami/user-service-pr-postgres --group storage-annotations.openchami.org --storage-version v1 --storage-type ent --db postgres --auth  (in fabrica-zoo/user-service)
🚀 Creating user-service-pr-postgres project...
...
+ ~/work/p/fabrica/bin/fabrica.exe add resource User  (in fabrica-zoo\user-service\user-service-pr-postgres)
...
+ ~/go/bin/goimports.exe -w apis\storage-annotations.openchami.org\v1\user_types.go  (in fabrica-zoo\user-service\user-service-pr-postgres)
+ ~/work/p/fabrica/bin/fabrica.exe generate  (in fabrica-zoo\user-service\user-service-pr-postgres)
🔧 Generating code...
...
+ go mod tidy  (in fabrica-zoo\user-service\user-service-pr-postgres)
...
```powershell
# in fabrica-zoo/user-service/user-service-pr-postgres/
$env:GOTOOLCHAIN = "go1.26.6"
go build ./...   # exit 0
```
