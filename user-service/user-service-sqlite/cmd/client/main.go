...
//
// This file provides a Cobra-based CLI for the user_service_pr_sqlite API.
...
//   - client health
//   - client user [list|get|create|update|patch|delete]
//
// Global flags (available for all commands):
//
//	--server       Server URL (env: USER_SERVICE_PR_SQLITE_SERVER)
//	--timeout      Request timeout (env: USER_SERVICE_PR_SQLITE_TIMEOUT)
//	--output, -o   Output format: table, json, yaml (env: USER_SERVICE_PR_SQLITE_OUTPUT)
//	--version, -v  API version to request: v1, v2beta1, etc. (env: USER_SERVICE_PR_SQLITE_VERSION)
//	--token        JWT bearer token (env: USER_SERVICE_PR_SQLITE_TOKEN)
//	--config       Config file path (default: ~/.user_service_pr_sqlite-cli.yaml)
//
// Configuration sources (in order of precedence):
//  1. Command-line flags
//  2. Environment variables (USER_SERVICE_PR_SQLITE_*)
//  3. Config file (~/.user_service_pr_sqlite-cli.yaml)
//  4. Default values
//
// Usage examples:
//
//	# List all users (default version)
//	client user list
//
//	# List users with specific version
//	client user list --version v2beta1
//
//	# Get User as v1
//	client user get <uid> --version v1
//
//	# Create from JSON file with version
//	cat user.json | client user create --version v2beta1
//
//	# Create from inline JSON
//	client user create --spec '{"name":"user-01","description":"Example User"}'
//
//	# Use environment variables
//	export USER_SERVICE_PR_SQLITE_SERVER=https://user_service_pr_sqlite.example.com
//	export USER_SERVICE_PR_SQLITE_VERSION=v2beta1
//	client user list
//
...
import (
	...
	"github.com/openchami/user-service-pr-sqlite/pkg/client"
...
var rootCmd = &cobra.Command{
	...
	Short: "user_service_pr_sqlite CLI",
	Long:  `A command-line interface for managing user_service_pr_sqlite resources.`,
}
...
func init() {
	...
	// Global flags
	rootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is $HOME/.user_service_pr_sqlite-cli.yaml)")
	rootCmd.PersistentFlags().StringVar(&serverURL, "server", "http://localhost:8080", "user_service_pr_sqlite server URL")
	...
	viper.SetEnvPrefix("USER_SERVICE_PR_SQLITE")
	viper.AutomaticEnv()
...
func initConfig() {
	if cfgFile != "" {
		...
		viper.SetConfigName(".user_service_pr_sqlite-cli")
	}
...
var userCmd = &cobra.Command{
	Use:   "user",
	Short: "Manage users",
	Long:  `Create, read, update, patch, and delete users.`,
}
...
Examples:
  # Create from stdin
  echo '{"username": "example-name", "email": "user@example.com", "password": "example-value", "fullName": "example-name", "role": "example-value", "active": true}' | client user create

  # Create with --spec flag
  client user create --spec '{"username": "example-name", "email": "user@example.com", "password": "example-value", "fullName": "example-name", "role": "example-value", "active": true}'
...
Examples:
  # Update from stdin
  echo '{"username": "example-name", "email": "user@example.com", "password": "example-value", "fullName": "example-name", "role": "example-value", "active": true}' | client user update <uid>

  # Update with --spec flag
  client user update <uid> --spec '{"username": "example-name", "email": "user@example.com", "password": "example-value", "fullName": "example-name", "role": "example-value", "active": true}'
...
`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		item, err := c.UpdateUser(ctx, args[0], req)
		if err != nil {
...
var userPatchCmd = &cobra.Command{
	Use:   "patch [uid]",
	Short: "Patch a User",
	Long: `Patch an existing User spec using various patch formats.
...
Examples:
  ...
  client user patch <uid> --spec '{"manufacturer":"Intel","model":"Updated Model"}'

  # Shorthand patch (dot notation - most convenient)
  client user patch <uid> --set manufacturer=Intel --set model="Updated Model" --unset customField

  # JSON Patch (RFC 6902 - most powerful)
  client user patch <uid> --json-patch '[
    {"op":"replace","path":"/manufacturer","value":"Intel"},
    {"op":"add","path":"/properties/newField","value":"newValue"}
  ]'

  # From stdin (JSON Merge Patch format)
  echo '{"manufacturer":"AMD","partNumber":"RYZEN-9000"}' | client user patch <uid>
...
Attempts to patch metadata or status fields will be ignored.`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		item, err := c.PatchUser(ctx, uid, patchData, contentType)
		if err != nil {
...
var userDeleteCmd = &cobra.Command{
	Use:   "delete [uid...]",
	Short: "Delete one or more User resources",
	Long: `Deletes one or more User resources.
...
Each UID is processed one by one as an individual, atomic delete operation. If any deletion fails, the command will still attempt to delete the remaining UIDs and report the errors at the end.`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		for _, uid := range args {
			if err := c.DeleteUser(ctx, uid); err != nil {
				errs = append(errs, fmt.Errorf("failed to delete User %q: %w", uid, err))
				continue
			}
...
func init() {
	...
	userCreateCmd.Flags().String("spec", "", "User specification in JSON format")
	userUpdateCmd.Flags().String("spec", "", "User specification in JSON format")

	// Add patch command flags
	userPatchCmd.Flags().String("spec", "", "JSON Merge Patch specification")
	userPatchCmd.Flags().String("json-patch", "", "JSON Patch operations (RFC 6902)")
	userPatchCmd.Flags().StringArray("set", nil, "Set field value using dot notation (field=value)")
	userPatchCmd.Flags().StringArray("unset", nil, "Unset field using dot notation")
	userPatchCmd.Flags().StringArray("add", nil, "Add value to array field (field=value)")
	userPatchCmd.Flags().StringArray("remove", nil, "Remove value from array field (field=value)")
}
