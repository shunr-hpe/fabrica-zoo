...
Examples:
  ...
  user_service_pr_sqlite export --format yaml --output ./backup

  # Export specific resource types
  user_service_pr_sqlite export --kinds User --output ./user-backup
`,
...
func exportResourceKind(ctx context.Context, kind, output, format string, perType bool) (int, error) {
	...
	switch kind {
	case "User":
		items, e := storage.Queryusers(ctx).All(ctx)
		if e != nil {
			return 0, fmt.Errorf("failed to query users: %w", e)
		}
